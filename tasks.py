...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-cae runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-cae.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ cae")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_cae.__version__))
    c.run("git push --tags")
